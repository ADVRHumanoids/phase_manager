...

{
    ...
    if (!rclcpp::ok())
    {
        std::cout << "ROS 2 not initialized. Initializing." << std::endl;
        int argc = 0;
        char ** argv = nullptr;
        rclcpp::init(argc, argv);
    }

    _node = std::make_shared<rclcpp::Node>("phase_manager_ros_server_class");
...
void RosServerClass::init_publishers()
{
    ...
    _timelines_pub = _node->create_publisher<phase_manager::msg::TimelineArray>("phase_manager/timelines", 10);
}
